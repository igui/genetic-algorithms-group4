...
class r0123456:
    ...
    def mutate(
        ...
        for o in offspring:
            ...
            a, b = self.rng.choice(len(o.tour), 2, replace=False)
    ...
    def selection(
        ...
        new_population = []
        for i in range(n):
            idx_sampled = self.rng.choice(len(pool), k, replace=False)  # k-tournament
    ...
    def optimize(self, filename):
        ...
        while True:
            ...
            # )
            if bestObjective < self.bestObjective:
...
if __name__ == '__main__':
    ...
    n_runs = 20
    seeds = np.random.randint(999, size=n_runs)
    bestObjectives = np.zeros(n_runs)
    for i, seed in enumerate(tqdm(seeds), 0):
        solver = r0123456(seed=seed)
        solver.optimize('tour50.csv')
        bestObjectives[i] = solver.bestObjective
    records = {'seeds': seeds, 'bestObjectives': bestObjectives}
    np.save('best_objectives_record-'+current_time, records)
